    ...
    def handle(self, *args, **options):
        ...
        for i in range(80):
            Client.objects.create(
                ...
                email          = faker.email(),
                employe        = Employe.objects.filter(is_superuser=False).order_by('?').first()
            )
        ...
        for i in range(100):
            ...
            for echeance in pret.echeances.all().order_by('date_echeance')[:faker.random_int(min=1, max=7)]:
                ...
                echeance.regler(montant, employe=Employe.objects.filter(is_superuser=False).order_by('?').first())
            ...
            for i in range(5):
                compte.deposer(montant=faker.random_int(min=10000, max=100000), employe=Employe.objects.filter(is_superuser=False).order_by('?').first(), mode = ModePayement.objects.filter().order_by('?').first())
                if faker.boolean(chance_of_getting_true=50):
                    try:
                        compte.retirer(montant=faker.random_int(min=10000, max=100000), employe=Employe.objects.filter(is_superuser=False).order_by('?').first(),  mode = ModePayement.objects.filter().order_by('?').first())
                    except ValueError:
                        pass
